...
Redis에 저장되는 모든 데이터의 구조와, 예매 과정에서 데이터가 어떻게 생성/변경/삭제되는지를 설명합니다.

---

## 1. 저장되는 데이터 종류 (3가지)

Redis에는 **5종류**의 데이터가 저장됩니다.
...
| `sessionId` | 이 좌석을 잡은 사용자의 고유 ID | `"sess_1709308800000_abc123def"` |
| `heldAt` | 잠금이 시작된 시각 | `"2026-03-02T10:00:00Z"` |
...
### ② 사용자별 잠금 목록 — `holds:{세션ID}`

> "이 사용자가 지금 잡아둔 좌석이 뭐가 있지?"

| 항목 | 설명 |
|------|------|
| **키 이름** | `holds:sess_1709308800000_abc123def` |
| **비유** | 사용자의 예매 메모장 — "내가 지금 잡아둔 좌석들" |
| **자동 삭제** | **7분 후** (좌석 잠금과 동시에 만료) |
...
[
  ...
  { "section": "A", "seat": 7, "afterParty": false }
]
```

이 목록은 두 가지 상황에서 사용됩니다:

1. **잠금 해제할 때** — 이 목록을 보고 어떤 좌석들을 풀어야 하는지 파악
2. **결제 생성할 때** — 이 목록을 보고 Shopify 장바구니에 담을 상품 결정

---

### ③ 결제 연결 정보 — `checkout:{장바구니ID}`

> "이 Shopify 장바구니가 어떤 사용자의 어떤 좌석인가?"
...
| 필드 | 의미 |
|------|------|
| `phase1.discount` | Phase 1 할인율 (0.2 = 20%) |
| `phase1.startDate` / `endDate` | Phase 1 적용 기간 |
| `phase1.enabled` | Phase 1 활성화 여부 |
| `phase2.discount` | Phase 2 할인율 (0.1 = 10%) |
...
```
사용자 브라우저
  └─ sessionId (localStorage에 저장, 새로고침해도 유지)
       │
       ├─ holds:{sessionId} ──────── 이 사용자가 잡은 좌석 목록
       │     │
       │     ├─ seat:A:5 ─────────── 개별 좌석 상태
       │     ├─ seat:A:6
       │     └─ seat:A:7
       ...
       └─ checkout:{cartId} ──────── Shopify 결제와 좌석 연결
             │
             └─ (결제 완료 시) seat:A:5, A:6, A:7 → "sold"로 변경
```
...
```
Redis 상태: (비어있음 — 모든 좌석 available)
```

브라우저에 세션 ID가 생성됩니다: `sess_170930_kim`

**② "결제하기" 버튼 클릭**

서버가 다음 데이터를 Redis에 저장합니다:

```
seat:A:5    → { status: "held", sessionId: "sess_170930_kim", tier: "premium", afterParty: true }   ⏰ 7분 후 삭제
seat:A:6    → { status: "held", sessionId: "sess_170930_kim", tier: "premium", afterParty: false }  ⏰ 7분 후 삭제
holds:sess_170930_kim → [{ section: "A", seat: 5, afterParty: true }, { section: "A", seat: 6, afterParty: false }]  ⏰ 7분 후 삭제
```
...
서버가 Shopify 장바구니를 만들고, 연결 정보를 저장합니다:

```
(기존 3개 유지)
checkout:gid://shopify/Cart/xyz789 → { sessionId: "sess_170930_kim", seats: [...], tier: "premium" }  ⏰ 30분 후 삭제
```
...
```
seat:A:5    → { status: "sold", sessionId: "sess_170930_kim", tier: "premium", afterParty: true }   ⏰ 삭제 안 됨 (영구 보존)
seat:A:6    → { status: "sold", sessionId: "sess_170930_kim", tier: "premium", afterParty: false }  ⏰ 삭제 안 됨 (영구 보존)
holds:sess_170930_kim → (삭제됨)
...
```
seat:C:1    → { status: "held", sessionId: "sess_170930_lee", ... }   ⏰ 7분 후 삭제
holds:sess_170930_lee → [{ section: "C", seat: 1, ... }]              ⏰ 7분 후 삭제
```
...
### 시나리오 3: 좌석 변경 (잡았다가 다른 좌석으로 변경)

> 박지훈님이 B-10을 잡았다가 마음이 바뀌어서 B-20으로 변경하는 경우

**① 처음 B-10 잠금**

```
seat:B:10   → { status: "held", sessionId: "sess_170930_park", ... }  ⏰ 7분 후 삭제
holds:sess_170930_park → [{ section: "B", seat: 10, ... }]            ⏰ 7분 후 삭제
```

**② B-20으로 변경하여 "결제하기" 클릭**

서버가 자동으로 기존 잠금을 해제하고 새 좌석을 잠급니다:

```
seat:B:10   → (삭제됨 → 다시 available)         ← 자동 해제
seat:B:20   → { status: "held", sessionId: "sess_170930_park", ... }  ⏰ 7분 후 삭제   ← 새로 잠금
holds:sess_170930_park → [{ section: "B", seat: 20, ... }]            ⏰ 7분 후 삭제   ← 목록 교체
```

**결과:** B-10은 즉시 풀리고, B-20이 새로 잠깁니다.
...
```
seat:A:5    → { status: "held", sessionId: "sess_170930_kim", ... }  ✅ 성공
holds:sess_170930_kim → [{ section: "A", seat: 5, ... }]
```
...
```
seat:A:6    → { status: "held", sessionId: "sess_other", ... }  ← 다른 사람이 이미 잡음
```
...
한 사람이 좌석을 독점하지 못하도록 서버에서 제한합니다:
...
| 항목 | 설명 |
|------|------|
| **생성 시점** | 처음 페이지에 접속할 때 자동 생성 |
| **형식** | `sess_1709308800000_abc123def` (타임스탬프 + 랜덤 문자) |
| **저장 위치** | 브라우저의 `localStorage` |
| **유지 기간** | 같은 브라우저를 사용하는 한 계속 유지 (새로고침해도 유지) |
| **다른 기기** | 다른 브라우저/기기는 별도의 세션 ID를 가짐 |

회원 가입이나 로그인은 필요 없습니다. 브라우저 단위로 자동 식별됩니다.
