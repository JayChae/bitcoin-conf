...
| **자동 삭제** | 임시 잠금(held): **30분 후 자동 삭제** / 판매 완료(sold): **삭제 안 됨** |
...
| `email` | 구매자 이메일 (sold일 때만) | `"buyer@example.com"` 또는 없음 |
...
3. 해당 좌석들을 `"sold"` (판매 완료)로 변경 + 구매자 이메일 저장
...
| `saleStatus` | 판매 상태 (`"upcoming"` = 판매 예정, `"open"` = 판매 진행, `"closed"` = 판매 마감) |
...
```
seat:A:5    → { status: "held", tier: "premium", afterParty: true }   ⏰ 30분 후 삭제
seat:A:6    → { status: "held", tier: "premium", afterParty: false }  ⏰ 30분 후 삭제
checkout:gid://shopify/Cart/xyz789 → { seats: [...], tier: "premium", phase: "earlybird1" }  ⏰ 3시간 후 삭제
```
...
```
seat:A:5    → { status: "sold", tier: "premium", afterParty: true, email: "user@example.com" }   ⏰ 삭제 안 됨 (영구 보존)
seat:A:6    → { status: "sold", tier: "premium", afterParty: false, email: "user@example.com" }  ⏰ 삭제 안 됨 (영구 보존)
...
> 이영희님이 좌석을 잡았지만 30분 안에 결제를 완료하지 않은 경우
...
```
seat:B:10   → { status: "held", tier: "premium", ... }  ⏰ 30분 후 삭제
```

**② 결제 포기 후 B-20으로 재시도**

B-10의 30분 TTL이 아직 남아있어도, 새로운 구매 요청은 독립적으로 처리됩니다:

```
seat:B:10   → { status: "held", ... }  ⏰ 남은 TTL 후 자동 삭제 (기존 유지)
seat:B:20   → { status: "held", tier: "premium", ... }  ⏰ 30분 후 삭제 (새로 잠금)
```

**결과:** B-10은 남은 TTL 후 자동 해제, B-20은 새로 잠금.
> 세션 ID 기반 자동 해제가 없으므로, 재시도 시 이전 좌석이 최대 30분간 불필요하게 잠길 수 있습니다.
...
| 키 | 자동 삭제 시간 | 이유 |
|----|--------------|------|
| `seat:*` (held) | **30분** | 결제하지 않은 사용자의 좌석을 자동 해제 |
| `seat:*` (sold) | **삭제 안 됨** | 판매된 좌석은 영구 보존 |
| `checkout:*` | **3시간** | 결제 처리에 충분한 여유 확보 (Shopify 알림 지연 + 재시도 대비) |
| `webhook:order:*` | **24시간** | Shopify 알림 중복 처리 방지용 |
| `pricing:config` | **삭제 안 됨** | 할인 설정은 어드민이 관리 |
| `pricing:phase2_sold:*` | **삭제 안 됨** | Phase 2 판매 카운터 |

> **레거시**: `/api/seats/hold` 엔드포인트를 사용하는 경우 `holds:{sessionId}` 키(30분 TTL)가 추가로 생성됩니다.
...
`pricing:config` 키도 없으면 코드의 **DEFAULT_CONFIG**가 사용됩니다:

```json
{
  "phase1": { "startDate": "...", "endDate": "...", "enabled": false },
  "phase2": { "maxTickets": { "vip": 10, "premium": 100, "general": 100 }, "enabled": false },
  "override": null,
  "saleStatus": "upcoming"
}
```
...
| 설정 | 초기값 | 의미 |
|------|--------|------|
| `saleStatus` | `"upcoming"` | 구매 버튼 비활성 (판매 예정) |
| `phase1.enabled` | `false` | EB1 할인 비활성 |
| `phase2.enabled` | `false` | EB2 할인 비활성 |
| `override` | `null` | 수동 오버라이드 없음 |
| → 결과 페이즈 | `"regular"` | 할인 없음 (정가) |

**즉, DB가 완전히 비어있으면 → "판매 예정 + 정가" 상태로 시작합니다.**
어드민에서 `saleStatus`를 `"open"`으로 변경해야 실제 판매가 시작됩니다.
