...
코드 변경은 `first-day-ticket` 브랜치에 완료되어 있습니다 (33개 파일, 타입 체크 통과).

> **가장 중요한 것 하나**: `SHOPIFY_VARIANT_MAINDAY` 환경변수가 없으면 **구매 버튼이 500으로 죽습니다.**
> 타입 체크·빌드는 멀쩡히 통과하고 실제 결제 시도에서만 터지므로, 배포 전에 반드시 확인하세요.
> (좌석은 즉시 해제되어 데이터가 깨지지는 않지만 고객이 결제를 못 합니다.)
...
- [ ] 그대로 둔다 (현재 상태)
- [ ] 11/8 이후 Main Day 스캔을 거부하도록 막는다 → 개발에게 요청
