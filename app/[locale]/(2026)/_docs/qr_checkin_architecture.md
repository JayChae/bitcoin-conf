...
Main Day는 11/7 하루만 유효합니다. 유효 일자와 뱃지 문구는 `TIER_VALID_DAYS`
([app/[locale]/(2026)/_constants/tickets.ts](../_constants/tickets.ts))에 선언하고,
스캐너가 스캔 성공 화면에 시안색 `NOV 7 ONLY · 11/7 전용` 뱃지를 띄웁니다.

**서버는 날짜를 검증하지 않습니다.** 11/7이 아닌 날에 Main Day QR을 스캔해도 초록 `WELCOME`이
뜨며, 입장 거부 여부는 뱃지를 본 현장 스태프가 판단합니다. 의도된 동작입니다 — 날짜별 입장 통제를
시스템에 넣는 것은 현 운영 규모에 과하다고 판단했습니다.

나중에 서버 차단이 필요해지면 `processCheckin`에서 `TIER_VALID_DAYS`와 `todayInKST()`를 대조하는
분기 하나로 충분하며, Redis 키 구조나 이미 발급된 QR은 건드리지 않습니다.
