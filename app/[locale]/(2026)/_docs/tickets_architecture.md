...
### [높음] HMAC 비교가 타이밍 공격에 취약

> [app/api/webhooks/shopify/route.ts:21](app/api/webhooks/shopify/route.ts#L21)

**HMAC 검증이란?**
Shopify가 보낸 Webhook 알림에는 "전자 서명"이 포함되어 있습니다.
서버는 이 서명을 직접 계산한 값과 비교해서, 진짜 Shopify가 보낸 건지 확인합니다.

**문제:**
현재 서명 비교에 일반 문자열 비교(`!==`)를 사용합니다.
일반 비교는 첫 글자부터 하나씩 확인하다가 틀린 글자를 만나면 즉시 멈춥니다.

```
서명 비교 과정:
  진짜 서명: "abcdef"
  시도 1:    "xbcdef" → 첫 글자에서 틀림 → 0.001초 만에 거절
  시도 2:    "abcxef" → 4번째에서 틀림  → 0.004초 만에 거절
                                          ↑ 시간 차이가 힌트가 됨
```

공격자가 수천 번 시도하면서 응답 시간을 측정하면, 서명을 한 글자씩 알아낼 수 있습니다.
실제 인터넷 환경에서는 어렵지만, 보안 모범 사례상 수정이 필요합니다.

**수정**: 모든 글자를 항상 끝까지 비교하는 `crypto.timingSafeEqual()`로 교체. 이렇게 하면 맞든 틀리든 비교 시간이 동일해서 힌트가 새어나가지 않습니다.
