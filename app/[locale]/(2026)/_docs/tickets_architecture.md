...
1. 좌석을 고르고 "구매하기"를 누르면 → 해당 좌석이 **30분간 임시로 잠기고** Shopify 결제 페이지로 이동
2. 잠긴 동안 다른 사람은 그 좌석을 선택할 수 없습니다
3. 30분 안에 결제하면 → 좌석이 **확정** 됩니다
4. 30분이 지나면 → 잠금이 **자동 해제**되어 다른 사람이 선택할 수 있습니다
...
- **TTL (자동 만료)**: "30분 후 자동 삭제" 기능이 내장. 결제를 안 하고 나간 사용자의 좌석을 시스템이 자동으로 풀어줍니다.
...
| **held** (임시 잠금) | 누군가 결제 중인 좌석 (30분 제한) | 회색 처리, 클릭 불가 |
...
④ "구매하기" 버튼 클릭 (로딩 스피너 표시)
   ...
   ↓
   ┌─ 성공: 좌석 30분간 잠금 + 즉시 Shopify 결제 페이지로 이동
...
⑤ Shopify 결제 페이지
   ...
   └─ 결제 포기/시간 초과: 30분 후 좌석 자동 해제 (Redis TTL)
```
...
| 키 형태 | 저장 내용 | 자동 삭제 | 비유 |
|---------|----------|----------|------|
| `seat:{섹션}:{번호}` | 좌석 상태 (held/sold, 티어, 애프터파티 여부, 이메일) | held: 30분 후 삭제 / sold: 삭제 안 됨 | 좌석에 붙인 "예약됨" 스티커 |
| `checkout:{장바구니ID}` | 결제 중인 좌석 목록 + 티어 + 구매 시점 페이즈 | 3시간 후 삭제 | 매표소 대기표 |
| `webhook:order:{주문ID}` | `"1"` (처리 완료 표시) | 24시간 후 삭제 | 중복 처리 방지 도장 |

**예시) `seat:A:5` 키에 저장되는 정보:**
```json
// held (임시 잠금):
{ "status": "held", "tier": "premium", "afterParty": true }

// sold (판매 완료 — 이메일 포함):
{ "status": "sold", "tier": "premium", "afterParty": true, "email": "buyer@example.com" }
```
...
Redis TTL(30분)이 자동 만료를 처리하므로, 세션 추적 없이도 시스템이 정상 동작합니다.
재시도 시 이전 좌석이 최대 30분간 잠기는 트레이드오프가 있지만, 컨퍼런스 규모에서 영향이 미미합니다.
...
- Shopify API 실패 시 → 500 응답 + 좌석 즉시 해제 (재시도 가능)
...
- **이벤트 타입 검증**: `x-shopify-topic` 헤더가 `orders/paid`인 경우만 처리. `orders/create`(결제 전) 등은 무시하여 미결제 주문의 좌석 확정을 방지
- **원자적 멱등성**: `SET NX`로 주문 ID를 선점하여, 동시에 도착한 중복 웹훅이 병렬 처리되는 것을 방지 (24시간 TTL)
- 장바구니 ID로 Redis에서 좌석 정보를 찾아 → **Lua 스크립트로 원자적 확인 후** "sold"(판매 완료)로 변경. 이미 다른 사용자가 구매한 좌석이 있으면 전체 거부 (더블부킹 방지)
- 구매자 이메일(`contact_email` 또는 `email`)을 Shopify 페이로드에서 추출하여 좌석 sold 데이터에 저장
- Phase 2 수량 카운터 증가: 체크아웃 생성 시 저장된 페이즈가 `earlybird2`이면 해당 티어의 판매 카운터를 `INCRBY`로 증가 (웹훅 시점이 아닌 구매 시점의 페이즈 기준)
- **좌석 확정 실패 시**: 500 반환 (Shopify 재시도 유도) + Discord 알림 발송 (환불 필요 안내)
...
- 인증 필요 (admin 쿠키)
- 13개 섹션 병렬 MGET으로 전체 좌석 스캔 (`getAllSeatSummary()`)
- 반환: 티어별 판매 요약 (total/sold/remaining), 전체 점유율, 애프터파티 수, 전체 구매 내역 (좌석번호, 티어, 애프터파티, 이메일)
- 어드민 페이지 예약 현황 탭에서 사용 (10초 간격 폴링)
